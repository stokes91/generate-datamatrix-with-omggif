...
class DataMatrix {
  ...
  renderGif() {
    const imageSizeEdge = (this.edgeLength + 4) * 2;
    ...
    this.pixels.forEach((that) => {
      const position = (that.y + 1) * 2 * imageSizeEdge + (that.x * 2) + 2 * imageSizeEdge + 4;

      allocdArray[position] = 1;
      allocdArray[position + 1] = 1;
      allocdArray[position + imageSizeEdge] = 1;
      allocdArray[position + imageSizeEdge + 1] = 1;
    });
