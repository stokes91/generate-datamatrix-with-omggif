...
function ToSet2(byte) {
  if (byte > 0x20 && byte < 0x30) {
    return byte - 0x21;
  }
  else if (byte > 0x39 && byte < 0x41) {
    return byte - 0x2b;
  }
  else if (byte > 0x5A && byte < 0x60) {
    return byte - 0x45;
  }
  return 0;
}
...
class DataMatrix {
  ...
  encodeC40(data) {
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (byte === 0x20 || (byte > 0x2f && byte < 0x3a) || (byte > 0x40 && byte < 0x5b)) {
        unpacked.push(ToX12(byte));
      }
      else if (byte < 0x20) {
        unpacked.push(0);
        unpacked.push(byte);
      }
      else if (byte > 0x20 && byte < 0x60) {
        unpacked.push(1);
        unpacked.push(ToSet2(byte));
      }
      else if (byte > 0x05f && byte < 0x80) {
        unpacked.push(1);
  ...
  renderGif() {
    const imageSizeEdge = (this.edgeLength + 2 + this.quietZone * 2) * this.scale;

    this.isoPattern();

    const allocdArray = new Array(imageSizeEdge * imageSizeEdge).fill(0);

    this.pixels.forEach((that) => {
      const position = (that.y + 1 + this.quietZone) * this.scale * imageSizeEdge + ((that.x + 1 + this.quietZone) * this.scale);

      if (this.scale <= 1) {
        allocdArray[position] = 1;
        return;
      }

      for (let yp = this.scale; yp--;) {
        for (let xp = this.scale; xp--;) {
          allocdArray[position + imageSizeEdge * yp + xp] = 1;
        }
      }
    });
    ...
    gif.addFrame(0, 0, imageSizeEdge, imageSizeEdge, allocdArray, {
      palette: [
        0xffffff,
        0x000000
      ]
